...
pair<Point, Point> getFurtherestPoint(vector<Point> &pts) {
    int N = size(pts);
    pair<Point, Point> res;
    for (int i = 0, j = 1; i < N; i++) {
        while ((j + 1) % N != i && area(pts[i], pts[(i + 1) % N], pts[j]) < area(pts[i], pts[(i + 1) % N], pts[(j + 1) % N])) {
            j = (j + 1) % N;
        }
        if (dist(res.first, res.second) < dist(pts[i], pts[j])) {
            res = { pts[i], pts[j] };
        }
    }
    return res;
